...
try:
    ...
    # Also check for firebase-service-account.json in project directory (functions/ or root/)
    if not firebase_config:
        functions_dir = os.path.dirname(os.path.abspath(__file__))
        root_dir = os.path.dirname(functions_dir)  # Parent directory (project root)
        
        # Check functions directory first
        default_config_path = os.path.join(functions_dir, 'firebase-service-account.json')
        if os.path.exists(default_config_path):
            firebase_config = default_config_path
            logger.info(f"Found Firebase config at: {default_config_path}")
        else:
            # Check root directory
            root_config_path = os.path.join(root_dir, 'firebase-service-account.json')
            if os.path.exists(root_config_path):
                firebase_config = root_config_path
                logger.info(f"Found Firebase config at: {root_config_path}")
            else:
                logger.warning(f"Firebase config not found. Checked:")
                logger.warning(f"  - {default_config_path}")
                logger.warning(f"  - {root_config_path}")
    
    if firebase_config:
        # Initialize Firebase Admin ONLY if config exists
        if os.path.exists(firebase_config):
            # Pass file path directly to Certificate - Firebase Admin SDK will handle reading it
            try:
                ...
                if "already exists" in str(e).lower():
                    ...
                    logger.error(f"File path: {firebase_config}")
                    logger.error(f"File exists: {os.path.exists(firebase_config)}")
                    # Try to read and validate the file manually for debugging
                    try:
                        with open(firebase_config, 'r') as f:
                            config_data = json.load(f)
                            logger.error(f"File type field: {config_data.get('type')}")
                            logger.error(f"File has required fields: type={config_data.get('type') is not None}, project_id={config_data.get('project_id') is not None}")
                    except Exception as read_err:
                        logger.error(f"Could not read file for debugging: {read_err}")
                    raise
            except Exception as e:
                logger.error(f"Unexpected error initializing Firebase Admin: {e}")
                logger.error(f"File path: {firebase_config}")
                raise
        else:
            ...
            try:
                ...
                # Validate that cred_dict has the required "type" field
                if not isinstance(cred_dict, dict):
                    logger.error(f"FIREBASE_CONFIG must be a JSON object, got: {type(cred_dict).__name__}")
                    logger.warning("Skipping invalid FIREBASE_CONFIG, will try Application Default Credentials")
                    firebase_config = None
                elif cred_dict.get('type') != 'service_account':
                    logger.error(f"Invalid FIREBASE_CONFIG: 'type' field must be 'service_account', got: {cred_dict.get('type')}")
                    logger.error(f"FIREBASE_CONFIG keys: {list(cred_dict.keys())}")
                    logger.warning("Skipping invalid FIREBASE_CONFIG, will try Application Default Credentials")
                    firebase_config = None
                else:
                    # Valid cred_dict - create certificate
                    try:
                        cred = credentials.Certificate(cred_dict)
                        firebase_admin.initialize_app(cred)
                        FIREBASE_AUTH_ENABLED = True
                        logger.info("Firebase Auth initialized from FIREBASE_CONFIG (authentication only)")
                    except ValueError as e:
                        if "already exists" in str(e).lower():
                            logger.info("Firebase Admin already initialized")
                            FIREBASE_AUTH_ENABLED = True
                            ...
                            logger.error(f"Firebase Admin initialization error: {e}")
                            logger.warning("Skipping FIREBASE_CONFIG due to initialization error, will try Application Default Credentials")
                            firebase_config = None
                    except Exception as e:
                        logger.error(f"Unexpected error initializing Firebase Admin from FIREBASE_CONFIG: {e}")
                        logger.warning("Skipping FIREBASE_CONFIG due to error, will try Application Default Credentials")
                        firebase_config = None
    # If firebase_config was invalid or not found, try Application Default Credentials
    if not firebase_config or not FIREBASE_AUTH_ENABLED:
        # No config file found or config was invalid - try to initialize with Application Default Credentials
        # This works in production (Firebase Functions/Cloud Run) where ADC is automatically provided
        try:
            firebase_admin.initialize_app()
            FIREBASE_AUTH_ENABLED = True
            logger.info("Firebase Auth initialized using Application Default Credentials")
        except Exception as adc_error:
            logger.warning(f"Could not initialize Firebase Admin with ADC: {adc_error}")
...
# Lazy initialization for Firestore client (for job tracking)
# Don't initialize at import time to avoid deployment timeouts
_firestore_db = None
_firestore_module = None

def get_firestore_db():
    """Lazy initialization of Firestore client - only called when needed"""
    global _firestore_db, _firestore_module
    ...
    try:
        if _firestore_module is None:
            ...
            _firestore_module = firestore_module
        
        if firebase_admin._apps:
            _firestore_db = _firestore_module.client()
            logger.info("Firestore client initialized (lazy)")
            return _firestore_db
        else:
...
firestore_db = None  # Will be set on first use
firestore = _firestore_module

# Lazy initialization for Google Cloud Storage client
# Don't initialize at import time to avoid deployment timeouts
_storage_client = None

def get_storage_client():
    """Lazy initialization of GCS client - only called when needed"""
    ...
    try:
        ...
        # Try to use service account file if available (for local development)
        firebase_config = os.environ.get('FIREBASE_CONFIG')
        if not firebase_config:
            functions_dir = os.path.dirname(os.path.abspath(__file__))
            root_dir = os.path.dirname(functions_dir)  # Parent directory (project root)
        ...
        if firebase_config and os.path.exists(firebase_config):
            # Use service account file for local development
            try:
                from google.oauth2 import service_account  # type: ignore
                # Try using from_service_account_file first (simpler)
                try:
                    cred = service_account.Credentials.from_service_account_file(firebase_config)
                    _storage_client = storage.Client(credentials=cred)
                    logger.info(f"Google Cloud Storage client initialized using service account file (bucket: {UPLOAD_BUCKET})")
                except Exception:
                    # Fallback to reading JSON and using from_service_account_info
                    with open(firebase_config, 'r') as f:
                        config_data = json.load(f)
                    cred = service_account.Credentials.from_service_account_info(config_data)
                    _storage_client = storage.Client(credentials=cred, project=config_data.get('project_id'))
                    logger.info(f"Google Cloud Storage client initialized using service account info (bucket: {UPLOAD_BUCKET})")
            except ImportError:
                # google.oauth2 not available, try setting GOOGLE_APPLICATION_CREDENTIALS env var
                os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = firebase_config
                _storage_client = storage.Client()
                logger.info(f"Google Cloud Storage client initialized using GOOGLE_APPLICATION_CREDENTIALS (bucket: {UPLOAD_BUCKET})")
        else:
            # Fall back to Application Default Credentials (for Cloud Run/Firebase Functions)
            # This will work in production where ADC is automatically configured
            _storage_client = storage.Client()
            logger.info(f"Google Cloud Storage client initialized using ADC (bucket: {UPLOAD_BUCKET})")
...
# For backwards compatibility
storage_client = None  # Will be set on first use
...
<body>
    ...
    <script>
        ...
        // New GCS-based upload flow: request signed URL → upload to GCS → poll status
        // This bypasses HTTP request size limits by uploading directly to Google Cloud Storage
        async function uploadFileWithBackgroundProcessing(file) {
            const progressFill = document.getElementById('progressFill');
            const progressText = document.getElementById('progressText');
            
            try {
                // Step 1: Request signed upload URL from backend
                devLog('[UPLOAD] Requesting signed URL for file:', file.name);
                progressText.textContent = 'Preparing upload...';
                ...
                if (!uploadUrlResponse.ok) {
                    if (uploadUrlResponse.status === 401 || uploadUrlResponse.status === 403) {
                        devLog('[UPLOAD] Authentication required');
                        document.getElementById('uploadArea').classList.remove('uploading');
                        pendingFile = file;
                        showLoginModal();
                        return;
                    }
                    const error = await uploadUrlResponse.json().catch(() => ({error: 'Failed to get upload URL'}));
                    throw new Error(error.error || 'Failed to get upload URL');
                }
                
                const { uploadUrl, jobId } = await uploadUrlResponse.json();
                devLog('[UPLOAD] Got signed URL, job ID:', jobId);
                devLog('[UPLOAD] Upload URL (first 100 chars):', uploadUrl.substring(0, 100) + '...');
                // Verify it's a GCS URL
                if (uploadUrl.includes('storage.googleapis.com') || uploadUrl.includes('storage.cloud.google.com')) {
                    devLog('[UPLOAD] ✓ Confirmed: Uploading to Google Cloud Storage');
                } else {
                    devError('[UPLOAD] ⚠ WARNING: Upload URL does not appear to be GCS:', uploadUrl.substring(0, 200));
                }
                
                // Helper function for chunked uploads (defined before use)
                async function uploadFileInChunks(uploadUrl, file, chunkSize, controller, progressBar, progressText) {
                    let uploadedBytes = 0;
                    
                    // Query upload status first to resume if needed
                    // Note: For resumable uploads, we can skip status query and start uploading from byte 0
                    // GCS will handle the resume automatically based on Content-Range headers
                    devLog('[UPLOAD] Starting fresh upload (GCS will handle resume automatically)');
                    uploadedBytes = 0;
                    ...
                    while (uploadedBytes < file.size) {
                        ...
                        // Only log every 10MB to reduce overhead
                        if (start % (10 * 1024 * 1024) < chunkSize) {
                            devLog(`[UPLOAD] Uploading bytes ${start}-${end - 1} of ${file.size} (${Math.round((start / file.size) * 100)}%)`);
                        }
                        ...
                        while (retries > 0 && !success) {
                            try {
                                // Upload chunk to GCS with explicit CORS mode
                                const chunkResponse = await fetch(uploadUrl, {
                                    ...
                                    // Explicitly set CORS mode and omit credentials for GCS direct upload
                                    mode: 'cors',
                                    credentials: 'omit'
                                });
                                ...
                                if (chunkResponse.status === 200 || chunkResponse.status === 201) {
                                    ...
                                    success = true;
                                    // Only log progress every 32MB (or every chunk for large files) to reduce overhead
                                    const logInterval = Math.max(chunkSize, 32 * 1024 * 1024);
                                    if (uploadedBytes % logInterval < chunkSize || uploadedBytes === file.size) {
                                        const percent = Math.round((uploadedBytes / file.size) * 100);
                                        const mbUploaded = (uploadedBytes / 1024 / 1024).toFixed(1);
                                        const mbTotal = (file.size / 1024 / 1024).toFixed(1);
                                        const elapsed = (Date.now() - (window.uploadStartTime || Date.now())) / 1000;
                                        const speed = elapsed > 0 ? (uploadedBytes / 1024 / 1024 / elapsed).toFixed(1) : '0';
                                        devLog(`[UPLOAD] Progress: ${percent}% (${mbUploaded}MB / ${mbTotal}MB) @ ${speed}MB/s`);
                                    }
                                } else if (chunkResponse.status === 503) {
                                    // Service unavailable, retry
                                    retries--;
                                    if (retries > 0) {
                                        devLog(`[UPLOAD] Service unavailable, retrying... (${retries} retries left)`);
                                        await new Promise(resolve => setTimeout(resolve, 1000 * (4 - retries))); // Exponential backoff
                                        continue;
                                    } else {
                                        const errorText = await chunkResponse.text().catch(() => 'Service unavailable');
                                    ...
                                    try {
                                        errorText = await chunkResponse.text();
                                    } catch (e) {
                                        errorText = chunkResponse.statusText || 'Unknown error';
                                    }
                                    const errorDetails = `Status: ${chunkResponse.status}, Response: ${errorText.substring(0, 200)}`;
                                    devError(`[UPLOAD] Upload failed: ${errorDetails}`);
                                    throw new Error(`Upload failed: ${chunkResponse.status} ${chunkResponse.statusText}. ${errorText.substring(0, 100)}`);
                                }
                            } catch (error) {
                                ...
                                devError(`[UPLOAD] Chunk upload error:`, error);
                                devError(`[UPLOAD] Error name: ${error.name}, message: ${error.message}, stack: ${error.stack?.substring(0, 200)}`);
                                
                                retries--;
                                if (retries > 0) {
                                    devLog(`[UPLOAD] Error uploading chunk, retrying... (${retries} retries left)`);
                                    await new Promise(resolve => setTimeout(resolve, 1000 * (4 - retries)));
                                } else {
                                    // Provide more detailed error message
                                    const detailedError = error.message || error.toString();
                                    throw new Error(`Upload failed after 3 retries: ${detailedError}`);
                                }
                            }
                        }
                        
                        // Update progress (0-50% for upload, 50-100% for processing)
                        const uploadProgress = Math.min(50, (uploadedBytes / file.size) * 50);
                        progressBar.style.width = uploadProgress + '%';
                        const percentComplete = Math.round((uploadedBytes / file.size) * 100);
                        const mbUploaded = (uploadedBytes / 1024 / 1024).toFixed(1);
                        const mbTotal = (file.size / 1024 / 1024).toFixed(1);
                        progressText.textContent = `Uploading to cloud storage... ${percentComplete}% (${mbUploaded}MB / ${mbTotal}MB)`;
                    }
                    
                    if (uploadedBytes < file.size) {
                        throw new Error('Upload incomplete: ' + uploadedBytes + '/' + file.size + ' bytes uploaded');
                    }
                }
                
                // Step 2: Upload file directly to GCS using resumable upload session
                ...
                devLog('[UPLOAD] Starting upload to GCS, file size:', file.size);
                
                const uploadController = new AbortController();
                // Use maximum chunk size (32MB) for very large files to minimize requests and maximize speed
                // GCS supports up to 32MB chunks - using maximum for best performance
                // For files > 200MB: 32MB chunks, > 100MB: 16MB, > 50MB: 8MB, > 10MB: 2MB, else: 256KB
                const CHUNK_SIZE = file.size > 200 * 1024 * 1024 ? 32 * 1024 * 1024 :
                                   file.size > 100 * 1024 * 1024 ? 16 * 1024 * 1024 : 
                                   file.size > 50 * 1024 * 1024 ? 8 * 1024 * 1024 :
                                   file.size > 10 * 1024 * 1024 ? 2 * 1024 * 1024 : 
                                   256 * 1024;
                // Calculate timeout based on file size and expected speed (20MB/s)
                // 462MB at 20MB/s = ~23 seconds, but allow 3x buffer for network variability
                const expectedSpeedMBps = 20; // Target: 20MB/second
                const estimatedSeconds = (file.size / 1024 / 1024) / expectedSpeedMBps;
                const MAX_UPLOAD_TIME = Math.max(600000, Math.ceil(estimatedSeconds * 3 * 1000)); // At least 10 min, or 3x estimated time
                const timeoutId = setTimeout(() => uploadController.abort(), MAX_UPLOAD_TIME);
                const numChunks = Math.ceil(file.size / CHUNK_SIZE);
                const estimatedTimeSeconds = Math.ceil(estimatedSeconds);
                devLog(`[UPLOAD] File: ${(file.size / 1024 / 1024).toFixed(2)}MB, Chunk size: ${(CHUNK_SIZE / 1024 / 1024).toFixed(2)}MB, Total chunks: ${numChunks}, Estimated time: ~${estimatedTimeSeconds}s at ${expectedSpeedMBps}MB/s`);
                
                try {
                    // For files > 1MB, use chunked uploads; otherwise single PUT
                    if (file.size > 1024 * 1024) {
                        devLog('[UPLOAD] Large file detected, using chunked upload');
                        await uploadFileInChunks(uploadUrl, file, CHUNK_SIZE, uploadController, progressFill, progressText);
                    } else {
                        devLog('[UPLOAD] Small file, using single PUT');
                    ...
                    if (!uploadResponse.ok && uploadResponse.status !== 308) {
                        ...
                        throw new Error(`Upload to storage failed: ${uploadResponse.status} ${uploadResponse.statusText}. ${errorText}`);
                    }
                    
                    if (uploadResponse.status === 308) {
                            devLog('[UPLOAD] Received 308 - upload incomplete, retrying with chunked upload');
                            await uploadFileInChunks(uploadUrl, file, CHUNK_SIZE, uploadController, progressFill, progressText);
                        } else {
                    devLog('[UPLOAD] File uploaded to GCS successfully, status:', uploadResponse.status);
                        }
                    }
                    ...
                    clearTimeout(timeoutId);
                    if (error.name === 'AbortError') {
                        const minutes = MAX_UPLOAD_TIME / 60000;
                        throw new Error(`Upload timeout after ${minutes} minutes. The file may be too large (${(file.size / 1024 / 1024).toFixed(1)}MB) or the connection is slow. Try a smaller file or check your internet connection.`);
                    }
                    devError('[UPLOAD] Upload error details:', error);
                    throw error;
                }
                progressText.textContent = 'Processing file...';
                progressFill.style.width = '50%'; // Upload complete, now processing
                ...
                devError('[UPLOAD] Upload error:', error);
                ...
                if (error.message.includes('CORS') || error.message.includes('Failed to fetch') || error.name === 'TypeError') {
                    userMessage = 'Upload failed: Network/CORS error. Please check your connection and try again. If the problem persists, the file may be too large.';
                } else if (error.message.includes('timeout') || error.name === 'AbortError') {
                    userMessage = 'Upload failed: Timeout. The file may be too large or your connection is slow. Try a smaller file or check your internet speed.';
                } else if (error.message.includes('403') || error.message.includes('401')) {
                    userMessage = 'Upload failed: Authentication error. Please refresh the page and try again.';
                }
                
                showUploadError(userMessage);
            }
        }
        ...
        async function pollJobStatus(jobId) {
            ...
            const poll = async () => {
                try {
                    ...
                    // For GCS uploads: upload is instant, so processing is 0-100%
                    // For direct uploads: upload was 0-50%, so processing is 50-100%
                    // Check if this is a GCS job (has jobId field) or old direct upload
                    const isGcsJob = status.jobId !== undefined;
                    const totalProgress = isGcsJob ? progress : (50 + (progress / 2));
...
def get_firebase_config():
    ...
    if not web_config:
        project_dir = os.path.dirname(os.path.abspath(__file__))  # functions directory
        root_dir = os.path.dirname(project_dir)  # project root
        # Support legacy locations and new organised config/ location
        candidate_paths = [
            os.path.join(project_dir, 'firebase-web-config.json'),
            os.path.join(root_dir, 'firebase-web-config.json'),
            os.path.join(root_dir, 'config', 'firebase-web-config.json'),
        ]
...
def get_upload_url():
    ...
    if not db or not storage_client:
        return jsonify({"error": "Firestore or GCS not available"}), 503

    file_name = request.json.get("fileName") or request.json.get("file_name")
    content_type = request.json.get("contentType") or request.json.get("content_type", "application/zip")
    file_size = request.json.get("fileSize") or request.json.get("file_size")
    ...
    # Use resumable upload session instead of signed URL
    # This works with ADC (Application Default Credentials) in Cloud Run/Firebase Functions
    # Signed URLs require a private key, but ADC only provides token-based credentials
    # Pass file_size if available to help GCS optimize the upload
    upload_url = blob.create_resumable_upload_session(
        content_type=content_type,
        size=file_size if file_size else None
    )
    
    logger.info(f"Generated GCS resumable upload URL for {file_name}: {upload_url[:100]}... (bucket: {bucket.name}, path: {blob.name})")
...
def upload_url():
    ...
    This endpoint creates a job record in Firestore and returns a signed URL
    that the client can use to upload files directly to GCS, bypassing the
    HTTP request size limits of Cloud Run/Firebase Functions.
    ...
    if not storage_client or not firestore_db:
        return jsonify({
            "error": "GCS or Firestore not configured. Please check environment setup."
        }), 503
    ...
    try:
        ...
        if file_size <= 0:
            return jsonify({"error": "file_size must be greater than 0"}), 400
        
        if file_size > MAX_UPLOAD_SIZE_MB * 1024 * 1024:
            return jsonify({
                "error": f"File size exceeds maximum of {MAX_UPLOAD_SIZE_MB}MB"
            }), 400
        
        # Validate file type
        filename_lower = file_name.lower()
        if not (filename_lower.endswith('.zip') or filename_lower.endswith('.json')):
            return jsonify({
                "error": "File must be a .zip or .json file"
            }), 400
        ...
        expires_at = created_at + EPHEMERAL_TTL
        ...
        job_data = {
            ...
            'user_id': user_id,
            'created_at': firestore.SERVER_TIMESTAMP if firestore else time.time(),
            'updated_at': firestore.SERVER_TIMESTAMP if firestore else time.time(),
            'expires_at': expires_at,
            'object_path': None,  # Will be set after upload
            'error_message': None
        }
        job_ref.set(job_data)
        logger.info(f"Created job {job_id} for file {file_name} (size: {file_size} bytes)")
        
        # Generate GCS object path
        # Sanitize filename to prevent path traversal
        safe_filename = os.path.basename(file_name).replace('/', '_').replace('\\', '_')
        object_path = f"uploads/{job_id}/{safe_filename}"
        ...
        logger.info(f"Generated resumable upload URL for job {job_id}, object: {object_path}")
...
def upload():
    """
    Handle ChatGPT export file upload - DEPRECATED for large files.
    
    This endpoint is kept for backwards compatibility but is restricted to small files
    (< 10MB). For larger files, use the new /upload-url endpoint which uploads
    ...
    # Restrict to 10MB for direct upload (larger files should use /upload-url)
    MAX_DIRECT_UPLOAD_SIZE = 10 * 1024 * 1024  # 10MB
    if file_size > MAX_DIRECT_UPLOAD_SIZE:
        return jsonify({
            "error": f"File too large for direct upload ({file_size / 1024 / 1024:.1f}MB). Maximum size: {MAX_DIRECT_UPLOAD_SIZE / 1024 / 1024:.0f}MB. Please use the new upload flow which supports files up to {MAX_UPLOAD_SIZE_MB}MB."
        }), 413
...
def upload_status(job_id):
    ...
    Reads job status from Firestore. If job is completed, updates the session
    with the processed data file path.
    """
    session_id = session.get('_id')
    user_id = session.get('user_id')
    
    # Try Firestore first (new GCS-based jobs)
    firestore_db = get_firestore_db()
    if firestore_db:
        try:
            job_ref = firestore_db.collection('jobs').document(job_id)
            ...
            if not job_doc.exists:
                # Fall back to in-memory jobs for backwards compatibility
                with job_lock:
                    job = processing_jobs.get(job_id)
                if not job:
                    return jsonify({"error": "Job not found"}), 404
                # Use old in-memory logic below
            else:
                # Firestore job found
                job_data = job_doc.to_dict()
                
                # Verify session ownership
                if job_data.get('session_id') != session_id or job_data.get('user_id') != user_id:
                    logger.warning(f"Unauthorized access attempt to job {job_id} by user {user_id}")
                    return jsonify({"error": "Unauthorized"}), 403
                
                # Check expiration
                expires_at = job_data.get('expires_at', 0)
                ...
                if job_data.get("status") == "completed":
                    storage_file = job_data.get("storage_file")
                    if storage_file and os.path.exists(storage_file):
                        session['conversations_file'] = storage_file
                        session['conversations_expires_at'] = expires_at
                        session.permanent = True
                        session.modified = True
            ...
            logger.exception(f"Error reading job {job_id} from Firestore: {e}")
            # Fall through to in-memory check
    
    # Fallback to in-memory jobs (for backwards compatibility with old upload flow)
    with job_lock:
        job = processing_jobs.get(job_id)
